...
int main()
{
    ...
    while (!asw::core::exit) {
        ...
        for (int i = 5; i >= 1; --i) {
            ...
            const uint8_t alpha = static_cast<uint8_t>(255 * (static_cast<float>(i) / 5.0F));
        ...
        for (int i = 0; i < 5; ++i) {
            const uint8_t a = static_cast<uint8_t>(50 + i * 40);
            asw::draw::rect_fill({ 560.0F + static_cast<float>(i) * 30.0F, 110.0F, 25.0F, 180.0F },
