...
asw::ui::Widget* asw::ui::Root::hit_test(
    Widget& w,
...
void asw::ui::Root::update() {
  ...
  if (mouse.change.x != 0.0f || mouse.change.y != 0.0f) {
    ...
    const UIEvent e{.type = UIEvent::Type::PointerMove,
                    .pointer_pos = mouse.position};
    dispatch_pointer(e);
  }
  ...
  if (get_mouse_button_down(MouseButton::Left) ||
      get_mouse_button_down(MouseButton::Right) ||
  ...
  // Button Up
  if (get_mouse_button_up(MouseButton::Left) ||
      get_mouse_button_up(MouseButton::Right) ||
