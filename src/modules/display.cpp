...
asw::Vec2<int> asw::display::get_logical_size()
{
    ...
    SDL_GetRenderLogicalPresentation(asw::display::renderer, &size.x, &size.y, nullptr);
    return size;
}
...
{
    SDL_SetRenderDrawBlendMode(asw::display::renderer, static_cast<SDL_BlendMode>(mode));
}
